...
// Capacity, Datetime, Wind_Forecast

const NESO_API_ENDPOINT = "https://api.neso.energy/api/3/action/";
...
async function shouldUpdate(feed) {
  ...
  const lastUpdated = lastUpdatedString ? new Date(lastUpdatedString) : new Date(2000, 1, 1);
...
export async function onRequest(context) {
  ...
  if (updated) {
    await env.WIND.put("data.json", JSON.stringify(data));
  }
